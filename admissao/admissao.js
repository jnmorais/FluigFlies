...
$(document).ready(function () {
  ...
  if (
      ...
      var thisModal = FLUIGC.modal(
        ...
        function (err, data) {
          var thisTable = FLUIGC.datatable("#postEmb", {
            ...
            header: [
              { title: "Cargo", size: "col-sm-8" },
              { title: "Processo", size: "col-sm-4" },
            ],
            multiSelect: false,
            search: {
              enabled: true,
    ...
    $("input[type='range']").on("input", function () {
      ...
      if ($(this).val() > 50) {
        ...
        $(this)
          .next("output")
          .text(comportamento[0] + " e " + comportamento[1])
        $(this)
          .next("output")
          .next("input")
          .val(comportamento[0] + " e " + comportamento[1])
  ...
  //Add Efetivção
  if (ATV == 37 || ATV == null) {
    show_on_click_custom(
      "rd_selecao_rh",
      "Sim, candidato selecionado pelo G&G",
...
function hide_on_load_custom(campo, valor1, valor2, valor3, valor4, show) {
  if (
    $("[name$='" + campo + "']:checked").val() == valor1 ||
    $("[name$='" + campo + "']:checked").val() == valor2 ||
    $("[name$='" + campo + "']:checked").val() == valor3 ||
    $("[name$='" + campo + "']:checked").val() == valor4
  ) {
    $("#" + show).show()
  } else {
    $("#" + show).hide()
...
function montaRadioEmail(emails) {
  ...
  $box.append(
    "Escolha uma das sugestões de email:" +
      "<br>" +
      "Caso não encontre sua sugestão, preencha o e-mail corporativo manualmente abaixo.</br>",
  )
