$(document).ready(function () {
    ...
    if (ATV) {
        ...
        // Caso de subst e Promoção (Efetivação de Estagiário pós desligamento) oculta campos NA e Infra
        $("input[name$='rd_mtvCntr']").click(function () {
            switch ($(this).val()) {
                case "Substituição de colaborador":
                case "Promoção (Efetivação de Estagiário pós desligamento)":
    ...
    if (ATV == 0 || ATV == 1 || ATV == 4) {
        /* Local data */
        var sistemas = ["Mega", "Expert", "Approvo", "Adobe/Doc Sign", "Construtor de vendas"];
    ...
    if (ATV >= 0 || ATV == null) {
        ...
        hide_on_load('rd_mtvCntr', 'Promoção (Efetivação de Estagiário pós desligamento)', null, 'rd_pc')
        hide_on_load('rd_mtvCntr', 'Promoção (Efetivação de Estagiário pós desligamento)', null, 'rd_etp_na')
    ...
    if ((ATV == 37 || ATV == 9) && gpLogado == "RecursosHumanos") {
        FLUIGC.toast({
